...
const Ecosystem = () => {
  ...
  // Fetch products excluding the user's company_id
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // Fetch products using the new endpoint that excludes the company ID
        const response = await fetch(`${API_URL}/products/exclude-company?companyId=${companyId}`);
        if (!response.ok) {
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        ...
        <div style={{ padding: 16, flex: 1, marginLeft: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '20px' }}>
            ...
            <Button onClick={() => navigate('/MyProducts')} style={{ marginRight: '10px' }}>
              View My Products
            </Button>
            <select
              className="form-select"
              onChange={handleBranchChange}
              style={{ backgroundColor: 'white', borderColor: '#ced4da', marginLeft: 20, marginRight: 70, maxWidth: 200 }}
            >
