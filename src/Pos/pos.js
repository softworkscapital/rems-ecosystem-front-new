...
function PosHome() {
  ...
  useEffect(() => {
    if (searchTerm) {
      const filteredSuggestions = availableGoods.filter(
        (good) =>
          (good.product_name &&
            good.product_name
              .toLowerCase()
          ...
          (good.product_code &&
            good.product_code.toLowerCase().includes(searchTerm.toLowerCase()))
      );
  ...
  const handleSearch = () => {
    const result = availableGoods.find(
      (good) =>
        (good.product_name &&
          good.product_name.toLowerCase() === searchTerm.toLowerCase()) ||
        (good.product_code &&
          good.product_code.toLowerCase() === searchTerm.toLowerCase())
    );
    if (result) {
  ...
  const handleAddGood = (good) => {
    ...
    setTotalAmount(
      totalAmount +
        good.selling_price * quantity +
        good.selling_price * vat * quantity
    );
    setSearchTerm("");
    setQuantity(1);
    setSuggestions([]);
  ...
  const handleCustomerChange = (e) => {
    ...
    if (selectedCustomer) {
      ...
      setBillingAddress(
        selectedCustomer.house_number_and_street_name +
          ", " +
          selectedCustomer.surbub
      );
  ...
  const BillingAddress = () => (
    <address>
      ...
      <br />
      {billingAddress || "No address selected"}
      <br />
      {customerCity}
      <br />
      {customerCountry}
    ...
    <div>
      ...
      <div
        ...
        style={{
          ...
          padding: "20px",
          marginTop: "40px",
          marginLeft: "250px", // Added margin to prevent sidebar overlap
        ...
        <div className="row col-md-12">
          <div className="col-md-8">
            <div className="col-md-12 mb-4">
              ...
              <div style={{ textAlign: "left" }}>Product: {searchTerm}</div>
                ...
                >
                  {suggestions.map((good, index) => (
                    <li
                      ...
                      onClick={() => handleSuggestionClick(good)}
                    >
                    ...
                    {selectedGoods.map((good, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          $
                          {(
                            good.selling_price * good.quantity +
                            good.selling_price * vat * good.quantity
                          ).toFixed(2)}
                        </td>
                      </tr>
                    ))}
          ...
          <div className="col-md-4">
            <div className="col-md-12">
              <div className="row mb-3">
                ...
                >
                  Total Cost: {totalAmount.toFixed(2)} {currency}
                </h4>
              </div>
